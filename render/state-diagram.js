...
// Thread-level chips grouped below each state node. Dashed arrows for transitions.
...
export function renderStateDiagram(ctx, processStates, previousStates, labelMap, colorMap) {
  ...
  // ── Arrows ────────────────────────────────────────────────────────────────
  _arrow(ctx, nx + R, ny, rx - R, ry, false, 'arrive',   cSubtle, cTertiary, fontMono);
  _arrow(ctx, rx + R, ry, ux - R, uy, false, 'dispatch', cSubtle, cTertiary, fontMono);
  _arrow(ctx, ux + R, uy, tx - R, ty, false, 'complete', cSubtle, cTertiary, fontMono);
  _arrow(ctx, ux, uy + R, wx, wy - R, false, 'block',    cSubtle, cTertiary, fontMono);
  _arcArrow(ctx, ux - R * 0.5, uy - R, rx + R * 0.5, ry - R, H * 0.04, true, 'preempt', cSubtle, cTertiary, fontMono);
  _arcArrow(ctx, wx - R * 0.7, wy + R * 0.5, rx + R * 0.3, ry + R * 0.8, H * 0.95, true, 'unblock', cSubtle, cTertiary, fontMono);
  ...
  for (const state of ['NEW', 'READY', 'RUNNING', 'WAITING', 'TERMINATED']) {
    ...
    ctx.fillText(state === 'TERMINATED' ? 'TERM' : state, cx, cy);
  }
  ...
  for (const ps of processStates) {
    ...
    if (threads) {
      for (const ts of threads) {
        ...
        const color = colorMap ? (colorMap.get(ts.tid) || pidToColor(ts.tid)) : pidToColor(ts.tid);
        if (byState[ts.state]) byState[ts.state].push({ label, color });
      }
    } else {
      const label = `P${ps.pid}`;
      const color = pidToColor(ps.pid);
  ...
  for (const state of ['NEW', 'READY', 'RUNNING', 'WAITING', 'TERMINATED']) {
    ...
    const [cx, cy] = pos[state];
    _drawChips(ctx, chips, cx, cy + R + 6, fontMono);
  }
}

function _drawChips(ctx, chips, centerX, startY, fontMono) {
  const rows = [];
  ...
  for (const row of rows) {
    ...
    for (const chip of row) {
      _roundRectSD(ctx, x, y, CHIP_W, CHIP_H, 2);
      ctx.fillStyle = chip.color;
      ctx.fill();
      ctx.font = `500 9px ${fontMono}`;
      ctx.fillStyle = contrastTextColor(chip.color);
      ctx.textAlign = 'center';
      ctx.textBaseline = 'middle';
...
function _arrow(ctx, x1, y1, x2, y2, dashed, label, stroke, labelColor, fontMono) {
...
function _arcArrow(ctx, x1, y1, x2, y2, bendY, dashed, label, stroke, labelColor, fontMono) {
  const cpX = (x1 + x2) / 2;
  const cpY = bendY;
