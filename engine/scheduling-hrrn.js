// scheduling-hrrn.js — Highest Response Ratio Next scheduling algorithm. Pure function, zero DOM.
...
/**
 * Selects the tid from readyQueue with the highest response ratio.
 * RR = (waitTime + burstTime) / burstTime, where waitTime = currentTime - arrivalTime.
 ...
 * @returns {number}
 */
function selectHighestRR(readyQueue, work, currentTime) {
  let bestIdx = 0;
  for (let i = 1; i < readyQueue.length; i++) {
    const best = work.get(readyQueue[bestIdx]);
    const cand = work.get(readyQueue[i]);
    const bestWait = currentTime - best.arrivalTime;
    const candWait = currentTime - cand.arrivalTime;
    const bestRR = (bestWait + best.burstTime) / best.burstTime;
    const candRR = (candWait + cand.burstTime) / cand.burstTime;
    if (
      candRR > bestRR ||
      (candRR === bestRR && cand.arrivalTime < best.arrivalTime) ||
      (candRR === bestRR && cand.arrivalTime === best.arrivalTime && cand.pid < best.pid) ||
      (candRR === bestRR && cand.arrivalTime === best.arrivalTime && cand.pid === best.pid && cand.tid < best.tid)
    ) {
      bestIdx = i;
    }
  }
...
export function runHRRN(processes) {
  ...
  while (time <= maxTime) {
    ...
    // ── Step 3: dispatch (highest response ratio from ready pool) ─────────────
    let contextSwitch = false;
    if (running === null && readyQueue.length > 0) {
      const next = selectHighestRR(readyQueue, work, time);
